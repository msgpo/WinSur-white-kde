...
install() {
  ...
  [[ -d ${SCHEMES_DIR}/McConcept10.colors ]] && rm -r ${SCHEMES_DIR}/McConcept10.colors
  ...
  [[ ${color} == '' ]] && \
  cp -ur ${SRC_DIR}/color-schemes/McConcept10.colors                                    ${SCHEMES_DIR} && \
  cp -ur ${SRC_DIR}/color-schemes/McConcept10.colors                                    ${PLASMA_DIR}/${name}/colors

  [[ ${color} == '-light' ]] && \
  cp -ur ${SRC_DIR}/color-schemes/McConcept10.colors                               ${SCHEMES_DIR}
}

echo "Installing 'McConcept10 kde themes'..."
